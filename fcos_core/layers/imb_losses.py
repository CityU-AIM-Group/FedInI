...
class SCELoss(nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)[:, 1:]
...
class RCELoss(nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        label_one_hot = torch.clamp(label_one_hot, min=1e-4, max=1.0)[:, 1:]
...
#     '''
#     Zhang and Sabuncu 'Generalized Cross Entropy Loss for Training Deep Neural Networks with Noisy Labels'
...
class GCELoss(nn.Module):
    ...
    def __init__(self, num_classes: int = 21, q: float = 0.7) -> None:
        super().__init__()
        self.q = q
        self.epsilon = 1e-9
...
if __name__ == '__main__':
    k = {'sd': NCEandRCE}
    a = k['sd'](num_classes=21)
    print(a(torch.rand(256,21), torch.ones(256).long()))